...
app.get('/api/dashboard/stats', authenticateToken, async (req, res) => {
  try {
    // Optimization: Run independent queries concurrently and use groupBy to reduce DB calls
    ...
    // Process KPI Stats
    const totalKPIs = kpiStats.reduce((sum, item) => sum + item._count._all, 0);
    const completedKPIs = kpiStats.find(s => s.status === 'completed')?._count._all || 0;
    const activeKPIs = kpiStats.find(s => s.status === 'active')?._count._all || 0;

    // Process Ticket Stats
    const totalTickets = ticketStats.reduce((sum, item) => sum + item._count._all, 0);
    const openTickets = ticketStats.find(s => s.status === 'open')?._count._all || 0;
    const inProgressTickets = ticketStats.find(s => s.status === 'in_progress')?._count._all || 0;
    const resolvedTickets = ticketStats.find(s => s.status === 'resolved')?._count._all || 0;
    const closedTickets = ticketStats.find(s => s.status === 'closed')?._count._all || 0;
