...
async function main() {
  ...
  const sampleTicket = await prisma.ticket.create({
    data: {
      ...
      description: 'Muhasebe modülünde yeni özellik talebi',
      priority: 'medium',
      status: 'open',
      sourceDepartment: 'Muhasebe',
      targetDepartment: 'Bilgi İşlem',
      createdBy: employeeProfile.id,
      creatorName: `${employeeProfile.firstName} ${employeeProfile.lastName}`,
      creatorEmail: employeeProfile.email
    }
  });
